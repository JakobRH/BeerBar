...
contract("BeerBar test", async accounts => {
  ...
  it("owner cannot open bar", async () => {
    await truffleAssert.reverts(beerBar.openBar({from: owner}));
    assert.equal(await beerBar.barIsOpen.call(), false);
  });
  ...
  it("others can cancel beer", async () => {
    await beerBar.addBarkeeper(barkeeper);
    await beerBar.openBar({from: barkeeper});

    let txOrdered = await beerToken.transfer(beerBar.address, 10, {from: partygoer});
    await assertEvent(txOrdered.receipt.transactionHash, 'BeerOrdered(address,uint256)');
    assert.equal(await beerToken.balanceOf.call(partygoer), 40);
    assert.equal(await beerBar.pendingBeer(partygoer), 10);

    let txCancelled = await beerBar.cancelOrder(10, {from: partygoer});
    await assertEvent(txCancelled.receipt.transactionHash, 'BeerCanceled(address,uint256)');
    assert.equal(await beerToken.balanceOf.call(partygoer), 50);
    assert.equal(await beerBar.pendingBeer(partygoer), 0);
  });
  ...
  it("served beer has to be burned by the token contract", async () => {
    await beerBar.addBarkeeper(barkeeper);
    await beerBar.openBar({from: barkeeper});

    let tokensBefore = (await beerToken.totalSupply.call()).toNumber();
    let tx = await beerToken.transfer(beerBar.address, 10, {from: partygoer});
    truffleAssert.eventEmitted(tx, 'Transfer');
    await assertEvent(tx.receipt.transactionHash, 'BeerOrdered(address,uint256)');
    assert.equal(await beerToken.balanceOf.call(partygoer), 40);
    await beerBar.serveBeer(partygoer, 10, {from: barkeeper});
    let tokensAfter = (await beerToken.totalSupply.call()).toNumber();
    assert.equal(tokensAfter, tokensBefore - 10)
  })
